...
module.exports = {
  name: "removeseller",
  category: "Config",
  description: "Removes A Seller From The Guild Seller List!",
  run: async (client, message, args, prefix) => {
    ...
    let embed = new MessageEmbed() .setColor(ee.wrongcolor) .setDescription(`<:ah_error:903498826688655381> Please Mention A User, Ex : \`!removeseller @Discord\``)
    ...
    guilds.findOne({ GuildID: message.guild.id}, async (err, guild) => { 
         if(!guild) {
          ...
          const mainmenu = new MessageEmbed() .setAuthor("Skid Alert | Remove Seller", ee.footericon) .setDescription(`<:ah_success:903498816362258462> | Successfully Removed <@${user.id}> From The Seller List`) .setColor(ee.color)
          return message.channel.send({ embeds: [mainmenu] })

         } else {
            
              array = arrayRemove(guild.Sellers, user.id)
              guild.Sellers = array
              guild.save()
